...
from claw_backend.assistant.schema import (
    ActionRoute,
    AssistantSession,
    OpenCodeWorkspace,
    RepoContext,
)
from claw_backend.pgoa.schema import ClusterProfile, HardwareInfo, SoftwareEnvironment
...
class BackendCliTests(unittest.TestCase):
    ...
    def test_run_pgoa_uses_openai_sdk_client_factory(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("claw_backend.cli.load_settings", return_value=settings), \
                 mock.patch("claw_backend.cli.build_openai_client", return_value="client") as build_client, \
                 mock.patch("claw_backend.cli.PGOAAgent") as agent_cls, \
    ...
    def test_discover_cluster_prints_cluster_profile_json(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("claw_backend.cli.load_settings", return_value=settings), \
                 mock.patch("claw_backend.cli.discover_cluster", return_value=profile), \
                 mock.patch("sys.stdout.write") as stdout_write:
    ...
    def test_discover_env_prints_software_environment_json(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            settings = AssistantSettings(pgoa_store_path=str(Path(tmpdir) / "experiments"))
            with mock.patch("claw_backend.cli.load_settings", return_value=settings), \
                 mock.patch("claw_backend.cli.discover_software_env", return_value=env_profile), \
                 mock.patch("sys.stdout.write") as stdout_write:
    ...
    def test_doctor_prints_guardrail_report(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            settings = AssistantSettings(pgoa_store_path=str(Path(tmpdir) / "experiments"))
            with mock.patch("claw_backend.cli.load_settings", return_value=settings), \
                 mock.patch("claw_backend.cli.build_doctor_report", return_value=report), \
                 mock.patch("sys.stdout.write") as stdout_write:
    ...
    def test_assist_prints_session_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("claw_backend.cli.load_settings", return_value=settings), \
                 mock.patch("claw_backend.cli.build_assistant_session", return_value=session), \
                 mock.patch("sys.stdout.write") as stdout_write:
